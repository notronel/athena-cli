...
def test_patient_search_uses_fhir_bundle_and_read_only_get() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path.endswith("/.well-known/smart-configuration"):
            return httpx.Response(200, json={"token_endpoint": "https://example.test/oauth2/v1/token"})
    ...
    assert requests[1].content == b"grant_type=client_credentials&scope=system%2FPatient.rs+system%2FDocumentReference.rs"
    assert requests[2].url.path == "/demo/Patient"
    assert requests[2].url.params["given"] == "Ada"
